#!/bin/bash
# Miaou notify script for OpenAI Codex CLI
...
#   notify = ["bash", "/path/to/miaou-codex-notify.sh"]

DEBOUNCE_FILE="/tmp/miaou-codex-notify-$$-debounce"
# Use parent PID (the codex process) so all invocations from the same
# session share the same debounce file.
DEBOUNCE_FILE="/tmp/miaou-codex-notify-${PPID}"
...
if pgrep -x Miaou > /dev/null 2>&1; then
    open -g "miaou://notify?target=${TARGET_ENCODED}&title=Codex+ready"
fi
